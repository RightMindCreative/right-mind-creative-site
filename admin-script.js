...
const setupPullToRefresh = (refresh) => {
  ...
  document.addEventListener("touchstart", (event) => {
    if (refreshing || window.innerWidth > 900 || window.scrollY > 0 || appView.hidden || event.touches.length !== 1) return;
  ...
  document.addEventListener("touchmove", (event) => {
    ...
    distance = Math.min(110, raw * 0.55);
...
setupPullToRefresh(loadApplications);
